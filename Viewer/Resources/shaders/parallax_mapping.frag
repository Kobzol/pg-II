...
{ 
    float height = texture(texDepth, texCoords).r;
...
vec2 steepParallax(vec2 texCoords, vec3 viewDir)
{
	...
	while (height < effectHeight)
	{
		...
		texCoords += texDelta;
		height = texture(texDepth, texCoords).r;
	}
...
void main()
{
	...
	vec4 diffValue = texture(texDiffuse, texCoords);
	vec3 normalValue = remapNormal(texture(texNormal, texCoords).xyz);
