...
void DemoGyroscope::initShaders()
{
	addResPath("shaders/");
...
void DemoGyroscope::initTextures()
{
	addResPath("models");
	//Load sprite textures
	GLuint texID;
	FIBITMAP *image = ImageManager::GenericLoader(getResFile("sand.bmp"), 0);
...
void DemoGyroscope::initMaterials()
{
...
void DemoGyroscope::initInfoEntities()
{
...
void DemoGyroscope::initSceneEntities()
{
	for (int i = 0; i < 3; i++)
	{
...
#pragma region Draw Scene Entities
	...
	model = glm::rotate(model, deltaAngle, glm::vec3(1.0f, 0.0f, 0.0f));
	model = glm::scale(model, glm::vec3(0.9f));
	m_sceneData->sceneEntities[SCENE_ENTITIES[0]]->m_modelMatrix = model;


	model = glm::rotate(model, deltaAngle, glm::vec3(0.0f, 0.0f, 1.0f));

	glm::quat rotation = glm::quat(glm::vec3(glm::radians(90.0f), glm::radians(0.0f), glm::radians(0.0f)));
	rotation = glm::normalize(rotation);
	model *= glm::mat4_cast(rotation);

	model = glm::scale(model, glm::vec3(0.5f));
	m_sceneData->sceneEntities[SCENE_ENTITIES[1]]->m_modelMatrix = model;


	model = glm::rotate(model, deltaAngle, glm::vec3(0.0f, 1.0f, 1.0f));
	model = glm::scale(model, glm::vec3(0.25f));
	m_sceneData->sceneEntities[SCENE_ENTITIES[2]]->m_modelMatrix = model;
